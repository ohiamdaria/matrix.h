...
int submatrix(matrix_t *A, matrix_t *copy, int rows_copy, int columns_copy) {
    int status = s21_create_matrix(A->rows - 1, A->columns - 1, copy), subi = 0;
    if (!status) {
        for (int i = 0; i < A->rows; i++) {
            int subj = 0;
            if (i == rows_copy) {
...
int minor(matrix_t *A, matrix_t *minor_matrix) {
    int status = s21_create_matrix(A->rows, A->rows, minor_matrix);
    if (!status) {
        for (int i = 0; i < A->rows; ++i)
            for (int j = 0; j < A->columns; ++j) {
                matrix_t copy = {0};
